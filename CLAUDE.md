...
- **Video Worker** (FFmpeg) → consumes jobs from Redis queue, processes videos, updates DB and storage
- **Database** (PostgreSQL) → users, channels, videos, comments, likes
- **Object Storage** (S3/MinIO) → video files and thumbnails (configured via MinIO)
- **Message Queue** (Redis / BullMQ) → video processing job queue
- **Email Service** (SMTP) → account confirmation and password recovery (Mailpit)
